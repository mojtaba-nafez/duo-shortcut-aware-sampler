...
#SBATCH --output=logs-train-slurm/duo-owt-time-dependent-%x-%j.out
#SBATCH --error=logs-train-slurm/duo-owt-time-dependent-%x-%j.err
...
srun -ul --cpu-bind=cores bash -c '
    ...
    python -u -m main \
        ...
        wandb.name=duo-owt-clean-loss-term \
        model=small \
        algo=duo \
        model.length=512 \
        ...
        +algo.clean_conf_lambda=0.1 \
        +algo.clean_conf_time_power=1.0 \
        +algo.clean_conf_t_threshold=0.7
'