...
#SBATCH --job-name=duo-owt-time-dependent-v2
...
srun -ul --cpu-bind=cores --environment=gidd bash -c '
    ...
    python -u -m main \
        ...
        wandb.name=duo-owt-clean-loss-term-0-02 \
        model=small \
        algo=duo \
        model.length=512 \
        ...
        +algo.clean_conf_lambda=0.02 \
        +algo.clean_conf_time_power=0.0 \
        +algo.clean_conf_t_threshold=1.0
'